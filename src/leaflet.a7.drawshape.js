...
(function (factory) {
	...
	L.A7DrawShape = L.Class.extend({
		...
		_createNode: function (latlng) {
			...
			const onMouseDown = (e) => {
				if (e.originalEvent.button === 0) { // Check for left mouse button
					L.DomEvent.stopPropagation(e);

					if (!isDragging && this.options.longPressDelete) { // Only start timer if not already dragging
						isLongPress = true;
			...
			const onMouseMove = (e) => {
				...
				isLongPress = false; // Cancel long press if dragging starts
				clearTimeout(longPressTimer); // Clear the timer if dragging starts
